...
#common argument
#[1]mainIPC String : main IPC class, for example "G06F"
#[2]limitedWord int 
#[3]dbName String : is database name
#[4]tbName String : is collection name
#[5]user String
#[6]passwd String
#[7]host String
#[8]port int
mainIPC = sys.argv[1]
limitedWord = int(sys.argv[2])
dbName = sys.argv[3]
tbName = sys.argv[4]
user = sys.argv[5]
passwd = sys.argv[6]
host = sys.argv[7]
port = int(sys.argv[8])
fileFrequencyWrite = open(sys.argv[9],'w')
writePCValue = open(sys.argv[10],'w')
...
#SelectContent is used to select content by head of line. Normally, we assume that the first two index of line will be a few of special characters which will indicate what's the content of current line
...
def checkRull(flag):
    if flag == "n" or flag == "l"  or flag == "vn"  or flag == "eng" or flag == "v" or flag =="q":
        return True
    return False
#function for create a interface of monge db
def createMongo():
...
def DataManage():
    # create the interface of mongoDB
    (conn,db,collection) = createMongo()
    # claim the global argument
    global mainIPC
    select_where = {"sampleMainIpc1":mainIPC}
    # collect data by sys.argv and construct data as default format with name file 
    for dataContent in collection.find(select_where):
        ...
        # loop the file and apply jieba function to divide and mark words
        for line in file:
            count=0
            wordList = list()
            flagList = list()
            ...
            if len(line)>1 and ((line[:2]=="#!" and line[:3] !='#!1') or line[:2]=="#*"):
                ...
                for word, flag in words:
                    count = count + 1
                # apply wordlist and flaglist to save the information. In this case, we will be able to get the pre-word/flag and next-word/flag. It will help us to select the right combination
                ...
                for i in range(len(wordList)):
                    ...
                    boolControl2 = False
                    pre = False
                    #considering about the different case of v, we choose the most useful format of word which includes v flag
                    if flagList[i] == "v":
                        ...
                        if not pre :
                            continue
                    #combine words by 2 and 3 amount. For either of them, we follow the different rule
                    if i+1<len(wordList) and (checkRull(flagList[i]) or flagList[i+1] == "a") and flagList[i+1][0]=="n" :
                    ...
                    if i+2<len(wordList) and checkRull(flagList[i]) and checkRull(flagList[i+1]) and flagList[i+2][0]=="n" :
...
def __init__():
    ...
    wordPCVcount = 0
    commentCount = 0
    # considering about the length of keyword, we rank them by their combination length
    ...
    evaluateCount = 0
    # provides 5 different PC-value values
    index = 0
    for key, value in sorted(PCV_dict.iteritems(), key=lambda (k,v): (v,k)): 
        ...
        if len(evaluateList)>0 and evaluateCount == evaluateList[0]:
            index = index + 1
            print("SuggestionValue " + str(index) + " "+str(value))
            evaluateList.pop(0)
