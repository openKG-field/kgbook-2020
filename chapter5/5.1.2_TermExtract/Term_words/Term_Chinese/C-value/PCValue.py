...
file = open('xmlResult2.txt',"r",encoding='utf8')
limitedWord =2
...
#calculate the c-value for each word

def PCValue(a, word_dict, file_dict, selection, wordTword, limitedWord):
    '''
    :param a:  当前词
    :param word_dict:  所有词与组合词并带有权重
    :param file_dict:  所有词组的文章频数
    :param selection:  是否存在嵌套可能【因为只有2组与3组,所有直接跟据词长判断】
    :param wordTword:  嵌套组合
    :param limitedWord: 最低词频限制
    :return:
    '''
    if selection == 'a':
        #X未被嵌套
        Va =math.log(2)* len(a) * word_dict[a]
        return Va
    elif selection =='b':
        ...
        if a in wordTword:
            ...
            Vb =math.log(2)* len(a) * word_dict[a]
    return Vb
...
def __init__():
    ...
    wordPCVcount = 0
    commentCount = 0
    for key, value in sorted(word_dict.items(), key=lambda k:k[1]):
    ...
    for key, value in sorted(file_dict.items(),key=lambda k:k[1]):
        if value >2 :
    ...
    evaluateCount = 0
    for key, value in sorted(PCV_dict.items(),key=lambda k:k[1]):
